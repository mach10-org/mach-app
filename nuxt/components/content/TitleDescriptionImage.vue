<template>
  <section>
    <div class="mx-auto items-center gap-8 py-8 container lg:grid lg:grid-cols-2 xl:gap-16">
      <nuxt-img v-if="imagePosition === 'left'" :src="imageSrc" :alt="imageAlt" :sizes="sizes" class="mb-4 hidden w-full rounded-lg lg:mb-0 lg:flex" />
      <div>
      ...
      <nuxt-img v-if="imagePosition === 'right' && !has2Images" :src="imageSrc" :alt="imageAlt" :sizes="sizes" class="mb-4 hidden w-full rounded-lg lg:mb-0 lg:flex" />
      <div v-else-if="imagePosition === 'right' && has2Images" class="grid grid-cols-2 mt-8 gap-4">
        <nuxt-img class="w-full rounded-lg" :src="imageSrc" :alt="imageAlt" :sizes="sizes2Images" />
        <nuxt-img class="mt-4 w-full rounded-lg lg:mt-10" :src="imageSrc2" :alt="imageAlt2" :sizes="sizes2Images" />
      </div>
    </div>
  </section>
</template>
