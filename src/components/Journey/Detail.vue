  ...
  <div class="border border-border-input rounded">
    <div @click="isOpen = !isOpen" :id="course.slug + '-heading'" class="group h-16 cursor-pointer flex w-full items-center px-4 bg-background-base rounded-2xl">
      ...
      <div class="grow flex items-center pl-2">
        <div class="w-full rounded-full h-2.5 group-hover:h-4 transition-height duration-200 bg-border-input">
          <div class="bg-gradient-to-r from-purple-400 to-pink-600 h-full rounded-full" :style="{ width: percentage + '%' }"></div>
        </div>
        <span class="text-right font-bold w-24 mx-8">{{ ((course.learning_lesson.length / course.quantity) * 100).toFixed(2) }}%</span>
      </div>
    ...
    <div v-show="isOpen" :id="'#' + course.slug + '-body'" :aria-labelledby="course.slug + '-heading'" class="md:grid-cols-2 mx-auto pb-6 px-6" :class="{ grid: isOpen }">
      <div class="flex items-center justify-center flex-col py-4 border-b md:border-r md:border-b-0 border-border-input">
        <Icon name="ChartBarIcon" :outline="false" classes="w-10 h-10 text-text-muted mb-1" />
        <h4 class="text-2xl text-text-title font-medium">{{ localJourney.detail_lastWeeks }}</h4>
        <p class="text-center">
          <strong>{{ qtyLastWeeks }} / {{ course.quantity }} ({{ ((qtyLastWeeks / course.quantity) * 100).toFixed(2) }}%)</strong>
          <span class="text-text-muted ml-2">{{ localJourney.detail_completed }}</span>
        </p>
      </div>
      <div class="flex items-center justify-center flex-col py-4">
        <Icon name="ArrowLeftOnRectangleIcon" :outline="false" classes="w-10 h-10 text-text-muted mb-1" />
        <h4 class="text-center text-2xl leading-7 font-medium">
          {{ moment(course.created_at).format('LL') }}
        </h4>
        <p class="text-center text-text-muted">{{ localJourney.detail_dateStart }}</p>
      </div>
      <div class="col-span-2 text-center pt-4">
        <a :href="`/courses/${course.slug}/`" class="link">{{ localJourney.detail_goToCourse }}</a>
