<template>
  <section class="flex flex-col min-h-[60vh] pt-16 pb-10 lg:pt-20 px-2">
    <div class="max-w-[450px] mx-auto">
      ...
      <div class="px-6">
        <OButton class="github-login" @click="signInWithGitHub" :disabled="statusOAuth.isLoading" expanded size="large">
          <span class="flex content-center justify-center">
            <svg
              v-if="statusOAuth.isLoading"
              aria-hidden="true"
              role="status"
              class="w-5 h-5 mr-auto flex self-center animate-spin"
              viewBox="0 0 100 101"
              fill="none"
            ...
            <svg v-else class="w-5 h-5 mr-auto flex self-center" viewBox="0 0 98 96" xmlns="http://www.w3.org/2000/svg">
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
            ...
            <span class="flex-1">Login with GitHub</span>
          </span>
        </OButton>
      </div>
    </div>
